...
int Scores::searchScore(const std::string& playerName) const {
    ...
    cout << "Player not found. Score is "; 
    return 0;
}
...
double Scores::averageMaxScores() const {
    ...
    for (int index = 0; index < entries.size(); index++) { // iterating through the vector of entries
        if (!(entries.at(index).getName().empty())) { // this is used to determine if the score at the index is empty (0) or not
            total += entries.at(index).getScore();
            totalEntries += 1;
        }
    }
...
std::pair<int, int> Scores::findMinMaxScores() const {
    ...
    for (int index = 1; index < entries.size(); index++) {
        if (tempMin > entries.at(index).getScore() && !(entries.at(index).getName().empty())) { // checking if score right after is lower than the temporary minimum. this statement also checks to make sure the score is valid (not 0)
            tempMin = entries.at(index).getScore(); // changes tempMin if the next score is found to be smaller
        }
        if (tempMax < entries.at(index).getScore() && !(entries.at(index).getName().empty())) { // checking if score right after is lower than the temporary maximum. this statement also checks to make sure the score is valid (not 0)
